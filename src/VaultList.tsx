...
export function VaultList() {
  ...
  const [redeemInput, setRedeemInput] = useState<RedeemInput>({ vaultId: '', amount: '' });
  ...
  const userTokens = useMemo(() => {
    ...
    const dsTokens = userCoins.data.filter((coin: CoinStruct) => 
  ...
  const handleRedeemDepegSwap = async (vault: SuiObjectResponse) => {
    ...
    if (BigInt(matchingPeggedToken.balance) < requiredPeggedAmount) {
      toast.error(`Insufficient pegged tokens. Need ${formatBalance(requiredPeggedAmount.toString(), PEGGED_DECIMALS)} but you have ${formatBalance(matchingPeggedToken.balance, PEGGED_DECIMALS)}`);
      return;
    }
    ...
    if (BigInt(dsToken.balance) < dsAmountToRedeem) {
      toast.error(`Insufficient DS tokens. Need ${formatBalance(dsAmountToRedeem.toString(), DS_DECIMALS)} but you have ${formatBalance(dsToken.balance, DS_DECIMALS)}`);
      return;
    }
    ...
    try {
      ...
      const [splitDsToken] = tx.splitCoins(
        tx.object(dsToken.coinObjectId),
                          ...
                          {status === 'Active' && hasRequiredTokens && (
                            <>
                              <input
                                ...
                                onChange={(e: ChangeEvent<HTMLInputElement>) => setRedeemInput({ 
                                  vaultId: vault.data?.objectId || '', 
                              ...
                              <Button 
                                ...
                                color="blue"
                                style={{ 
                                  cursor: (isTransactionPending || !redeemInput.amount) ? 'default' : 'pointer'
                                }}
                              >
