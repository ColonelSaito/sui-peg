...
import { Shield, Wallet, CoinsIcon, DollarSign, Landmark, AlertTriangle, ArrowRight } from "lucide-react"

interface TokenFlowIllustrationProps {
  type: "underwriter" | "hedger" | "maturity"
}

export default function TokenFlowIllustration({ type }: TokenFlowIllustrationProps) {
    ...
    return (
      <div className="relative h-[300px] md:h-[400px] w-full bg-gray-900/30 rounded-xl border border-gray-800">
        ...
        <div className="absolute top-6 left-6 flex flex-col items-center">
          ...
          <span className="text-sm font-medium text-purple-300">Underwriter</span>
        </div>
        ...
        <div className="absolute inset-x-0 bottom-6 px-6">
          <div className="bg-gray-800/70 rounded-lg p-4 border border-gray-700">
            <div className="flex items-center justify-between mb-6">
              {/* Step 1: Supply SUI */}
              <div className={`flex flex-col items-center ${step === 0 ? "opacity-100" : "opacity-40"}`}>
                <div className="bg-cyan-900/50 p-2 rounded-full mb-1 border border-cyan-500/30">
                  <CoinsIcon className="h-6 w-6 text-cyan-300" />
                </div>
                <span className="text-xs font-medium text-cyan-300">SUI</span>
              </div>

              <ArrowRight className={`h-5 w-5 text-gray-400 ${step === 0 ? "text-cyan-400" : ""}`} />

              {/* Step 2: Supply sSUI */}
              <div className={`flex flex-col items-center ${step === 1 ? "opacity-100" : "opacity-40"}`}>
                <div className="bg-green-900/50 p-2 rounded-full mb-1 border border-green-500/30">
                  <CoinsIcon className="h-6 w-6 text-green-300" />
                </div>
                <span className="text-xs font-medium text-green-300">sSUI</span>
              </div>

              <ArrowRight className={`h-5 w-5 text-gray-400 ${step === 1 ? "text-green-400" : ""}`} />

              {/* Step 3: Receive Depeg Tokens */}
              <div className={`flex flex-col items-center ${step === 2 ? "opacity-100" : "opacity-40"}`}>
                <div className="bg-purple-900/50 p-2 rounded-full mb-1 border border-purple-500/30">
                  <Shield className="h-6 w-6 text-purple-300" />
                </div>
                <span className="text-xs font-medium text-purple-300">Depeg Tokens</span>
              </div>
            </div>

            <div className="text-center text-sm text-gray-300">
              {step === 0 && "Step 1: Underwriter supplies SUI to the vault"}
              {step === 1 && "Step 2: Underwriter supplies sSUI to the vault"}
          ...
          <div className="relative w-[200px] h-[100px]">
            ...
            <div
              className={`absolute left-0 top-0 transition-all duration-1000 ease-in-out ${
                step >= 0 ? (step > 0 ? "opacity-0 translate-x-[100px]" : "opacity-100") : ""
              }`}
            >
            ...
            {/* sSUI Token Animation */}
            <div
              className={`absolute left-0 bottom-0 transition-all duration-1000 ease-in-out ${
                step >= 1 ? (step > 1 ? "opacity-0 translate-x-[100px]" : "opacity-100") : "opacity-0"
              }`}
            >
        ...
        <div className="absolute inset-x-0 bottom-6 px-6">
          <div className="bg-gray-800/70 rounded-lg p-4 border border-gray-700">
            <div className="flex items-center justify-between mb-6">
              ...
              <ArrowRight className={`h-5 w-5 text-gray-400 ${step === 0 ? "text-blue-400" : ""}`} />
            ...
            <div className="text-center text-sm text-gray-300">
              ...
              {step === 1 && "Step 2: Hedger pays sSUI to the vault"}
              {step === 2 && "Step 3: Hedger receives depeg tokens for protection"}
            </div>
          </div>
        </div>
      ...
      <div className="absolute inset-x-0 bottom-6 px-6">
        <div className="bg-gray-800/70 rounded-lg p-4 border border-gray-700">
          <div className="flex items-center justify-between mb-6">
            ...
            <ArrowRight className={`h-5 w-5 text-gray-400 ${step === 1 ? "text-blue-400" : ""}`} />
            ...
            <div className={`flex flex-col items-center ${step === 2 ? "opacity-100" : "opacity-40"}`}>
              ...
              <span className="text-xs font-medium text-purple-300">Underwriter</span>
            </div>
          </div>

          <div className="text-center text-sm text-gray-300">
            {step === 0 && "Before Maturity: If a depeg event occurs"}
            {step === 1 && "Hedgers can claim SUI using their depeg tokens"}
            {step === 2 && "After Maturity: Underwriters claim all remaining tokens"}
          </div>
        </div>
      </div>
